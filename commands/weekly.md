...
1. Read `A/HANDOFF.md`, `A/config.json`, and the week just finished plus the next one in the
   active cycle's plan, `A/cycles/<config.activeCycle>/plan.json`.
...
8. **Verdict: progress, repeat the week, or stop.** One line with the why. If it's repeat or stop,
   propose the exact change to the plan and **wait for the athlete's OK** before writing it
...
11. If this was the plan's last week (or the race just happened), point to `/strata:goal new`.
